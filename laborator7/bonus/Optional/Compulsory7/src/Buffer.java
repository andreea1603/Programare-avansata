...
public class Buffer {
    ...
    public synchronized void put (int number, int strategy ) {
        ...
        if(tokens.size()>0)
        {
            ...
            if(strategy==0)
            {
                ...
                if(k==-1) {
                    k = block();
                    System.out.println("Nu am gasit ceva pentru mine");
                }
                if(k==-1) {
                    k = random();
                    System.out.println("Nu am gasit ceva sa blochez");
                }
                nrToken=k;
                System.out.println("\t\nA ales tokenul: " + tokens.get(nrToken));
                System.out.println("\n");
    ...
    int build(){
        for( Pair k : asignedTokens.keySet()) {
            if(asignedTokens.get(k).equals(turn)){
                for (int i = 0; i < tokens.size(); i++) {
                    if(tokens.get(i).first==k.second)
                        return i;
                }
            }
        }
    ...
    int random(){
        return (int)(Math.random()*10000)%tokens.size();
    }
